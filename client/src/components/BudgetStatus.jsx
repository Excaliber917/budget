...
function BudgetStatus() {
    ...
    return (
        <div className="mb-8">
            ...
            <Link to='/budgetitems' className="mb-5 text-sm text-blue-600 italic cursor-pointer hover:underline hover:text-blue-700">click here to see the full list</Link>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {budgetList?.length !== 0 ? (
                    budgetList.map((budget) => {
                        const spent = calculateAmount(budget.category, budget.startDate, budget.endDate);
                        const percentage = Math.min((spent / budget.amount) * 100, 100); // Calculate percentage, cap at 100%
                        ...
                        return (
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 flex flex-col items-center" key={budget._id}>
                                ...
                                <p className="text-gray-600 dark:text-gray-400">${spent.toFixed(2)} / ${budget.amount.toFixed(2)}</p>
                            </div>
                        );
                    })
                ) : (
                    <>
                        ...
                        <div className="dark:bg-gray-800 h-64 bg-gray-300 shadow-md rounded flex items-center justify-center text-xl dark:text-gray-500">
                            No budget set yet
                        </div>
                        <div className="dark:bg-gray-800 h-64 bg-gray-300 shadow-md rounded flex items-center justify-center text-xl dark:text-gray-500">
                            No budget set yet
